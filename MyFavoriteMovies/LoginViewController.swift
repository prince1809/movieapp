...
class LoginViewController: UIViewController {
    ...
    private func getRequestToken() {
        ...
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            guard (requestToken  != nil) else {
                displayError("Could not parse the data as json: '\(Constants.TMDBResponseKeys.RequestToken)' in \(parsedResult)")
                return
            }
    ...
    private func loginWithToken(requestToken: String) {
        ...
        /* 4. Make the request */
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, reposnse, error) in
            ...
            guard let statusCode = (reposnse as? NSHTTPURLResponse)?.statusCode where statusCode >= 200 && statusCode <= 229 else {
                displayError("Your request comes with a status code rather tham 2XX!")
                return
            }
            ...
            guard let data = data else {
             ...
             /* 5. Parse the data */
            let parsedDResult: AnyObject!
            do {
                parsedDResult = try NSJSONSerialization.JSONObjectWithData(data, options: .AllowFragments)
            } catch {
                displayError("Could not parse the data as JSON: '\(data)'")
                return
            }
            ...
            guard let success = parsedDResult[Constants.TMDBResponseKeys.Success] as? Bool where success == true else {
                displayError("Cannot find key '\(Constants.TMDBResponseKeys.Success)' in \(parsedDResult)")
                return
            }
    ...
    private func getSessionID(requestToken: String) {
        ...
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            /* GUARD if there was an eror */
            guard error == nil else {
                displayError("No data was returned by request")
                return
            }
            
            /* GUARD: Did we get a 2xx response */
            guard let statusCode = (response as? NSHTTPURLResponse)?.statusCode where statusCode >= 200 && statusCode <= 229 else {
                displayError("Your request comes with a status code rather tham 2XX!")
                return
            }
            ...
            let session_id = parsedResult[Constants.TMDBResponseKeys.SessionID] as? String
            guard session_id != nil else {
                displayError("Could not parse the data as json: '\(Constants.TMDBResponseKeys.SessionID)' in \(parsedResult)")
                return
            }
    ...
    private func getUserID(sessionID: String) {
        ...
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            /* GUARD: Check if there was any error  */
            guard error == nil else {
            ...
            let userID = parsedResult[Constants.TMDBResponseKeys.UserID] as? Int
